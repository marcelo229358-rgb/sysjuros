...
export function arredondarMoedaAmort(valor: number): number {
...
export function redistribuirSaldoPendentes(
  ...
  const total = Math.max(0, arredondarMoedaAmort(novoTotal));
  const valorParcela = arredondarMoedaAmort(total / pendentes.length);
  let soma = 0;
  const resultado: ParcelaAmortizada[] = [];

  for (let i = 0; i < pendentes.length; i++) {
    const isUltima = i === pendentes.length - 1;
    const valor = isUltima ? arredondarMoedaAmort(total - soma) : valorParcela;
    soma += valor;
    resultado.push({
      id: pendentes[i].id,
      valorOriginal: valor,
...
export function calcularAmortizacao(
  ...
  const valorAmortizar = arredondarMoedaAmort(valor);
  ...
  if (modo === 'PARCELA_ESPECIFICA') {
    const alvo =
      ordenadas.find((p) => p.numero === parcelaNumero) ?? ordenadas[0];
    const novoValor = Math.max(0, arredondarMoedaAmort(alvo.valorOriginal - valorAmortizar));
  ...
  const novoTotal = Math.max(0, arredondarMoedaAmort(totalAtual - valorAmortizar));
